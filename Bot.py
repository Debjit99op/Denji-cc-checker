...
token = '7231905322:AAEbOGnMB1z9aH1_xhVAlxtS9GITn9a8YVU'
...
def start(message):
    ...
    if str(message.chat.id) not in subscriber:
        bot.reply_to(message, "❌ Only For authorized users! Contact Admin @Denji40")
        return
    bot.reply_to(message, "<b>Welcome To Mass CC Checker\n Please send the Combo For check . \n\n Must Join @Denji40\nDeveloped By @Denji40 </b>")

@bot.message_handler(content_types=["document"])
def main(message):
    """Handler for processing uploaded files"""
    if str(message.chat.id) not in subscriber:
        bot.reply_to(message, "❌ Access Denied! Contact Admin @Denji40")
        return
    ...
    try:
        with open("combo.txt", 'r') as file:
            ...
            for card in card_lines:
                ...
                bot.edit_message_text(chat_id=message.chat.id, message_id=processing_msg, 
                                      text=f"🔄 Processing Cards...\nPowered by @Denji40 | Credit: @Denji40", 
